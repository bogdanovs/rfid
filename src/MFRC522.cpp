...
MFRC522::MFRC522(	byte chipSelectPin,		///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
					byte resetPowerDownPin,	///< Arduino pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low). If there is no connection from the CPU to NRSTPD, set this to UINT8_MAX. In this case, only soft reset will be used in PCD_Init().
					byte spi_id, byte spi_sclk, byte spi_miso, byte spi_mosi, byte spi_cs
				) {
	_chipSelectPin = chipSelectPin;
	_resetPowerDownPin = resetPowerDownPin;

	MFRC522_CS = spi_cs;
	MFRC522_MISO = spi_miso;
	MFRC522_MOSI = spi_mosi;
	MFRC522_SCLK = spi_sclk;

	spi = new SPIClass(spi_id);
	spi->begin(MFRC522_SCLK, MFRC522_MISO, MFRC522_MOSI, MFRC522_CS);
} // End constructor
